import React from 'react'

export default function FavCar(props) {
  return (
    <>
      <svg width='1em' height='1em' {...props}>
        <g id='SVGRepo_bgCarrier' strokeWidth='5' />

        <g id='SVGRepo_iconCarrier'>
          <path
            fill='currentColor'
            d='M30.9,20.2c-0.7-0.8-1.7-1.2-2.7-1.2c-1,0-2,0.4-2.7,1.2l-0.9,1l-0.9-1c-0.7-0.8-1.7-1.2-2.7-1.2c-1,0-2,0.4-2.7,1.2
	c-1.5,1.5-1.5,4,0,5.6l5.7,5.9c0.2,0.2,0.5,0.3,0.7,0.3s0.5-0.1,0.7-0.3l5.7-5.9C32.3,24.2,32.3,21.7,30.9,20.2z'
          />
          <path
            fill='#fff'
            d='M15.1,22H8.4l1.2-2.3c0.5-1,1.5-1.7,2.7-1.7h3.5c0.5,0,1,0.1,1.4,0.3c1-0.9,2.3-1.3,3.7-1.3c0,0,0.1,0,0.1,0c0,0,0,0,0.1,0
	h3c0.5,0,1,0.4,1,1c0.9-0.6,1.9-0.9,3-1v0c0-1.6-0.8-3.1-2-4h1c0.6,0,1-0.4,1-1s-0.4-1-1-1h-2.8L23,8c-0.8-1.8-2.6-3-4.6-3H9.6
	C7.6,5,5.8,6.2,5,8l-1.3,3H1c-0.6,0-1,0.4-1,1s0.4,1,1,1h1c-1.2,0.9-2,2.4-2,4v4c0,0.9,0.4,1.7,1,2.2V25c0,1.7,1.3,3,3,3h2
	c1.7,0,3-1.3,3-3v-1h6.1C15,23.3,15,22.7,15.1,22z M7,19H4c-0.6,0-1-0.4-1-1s0.4-1,1-1h3c0.6,0,1,0.4,1,1S7.6,19,7,19z M5.5,12
	l1.4-3.2C7.4,7.7,8.4,7,9.6,7h8.7c1.2,0,2.3,0.7,2.8,1.8l1.4,3.2H5.5z'
          />
        </g>
      </svg>
    </>
  )
}
